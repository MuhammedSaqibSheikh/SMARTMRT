...
namespace SMARTMRT
{
    public partial class Spare_Inventory : Telerik.WinControls.UI.RadForm
    {
        public Spare_Inventory()
        {
            InitializeComponent();
        }
        ...
        private void Spare_Inventory_Load(object sender, EventArgs e)
        {
        ...
        public void Machine_Spare()
        {
            ...
            {
                query = "select s.V_SPARE_SUB_ID,s.V_SPARE_SUB_DESC,s.I_QUANTITY from SPARE_SUB_CATEGORY s ";
            }
            else
            {
                query = "select s.V_SPARE_SUB_ID,s.V_SPARE_SUB_DESC,s.I_QUANTITY from SPARE_SUB_CATEGORY s,SPARE_MAIN_CATEGORY sm where sm.V_SPARE_MAIN_DESC='" + cmbmachine.Text + "' and s.V_SPARE_MAIN_ID=sm.V_SPARE_MAIN_ID";
            }
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                dgvmachine.Rows.Add(spareid, sparedesc, quantity, balance);
                data1.Rows.Add(cmbmachine.Text, spareid, sparedesc,quantity, balance);
            }
        }

        private void Spare_Inventory_Initialized(object sender, EventArgs e)
        {
        ...
        private void btnreport_Click(object sender, EventArgs e)
        {
            if (btnreport.Text == "Report View")
            {
                ...
                reportViewer1.LocalReport.ReportEmbeddedResource = "SMARTMRT.Spare_Inventory.rdlc";
